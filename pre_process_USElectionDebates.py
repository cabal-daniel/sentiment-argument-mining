...
def read_us_election_corpus():
    ...
    indices = [
        int(re.sub(r".*\/(\d+)_(\d+).*", "\g<1>", File)) for File in files_raw
    ]
    files_raw = [f for i, f in sorted(zip(indices, files_raw))]
    files_ann = glob("./data/USElectionDebates/*.ann")
    indices = [
        int(re.sub(r".*\/(\d+)_(\d+).*", "\g<1>", File)) for File in files_ann
    ]
    ...
    for i in range(len(flat_text)):
        ...
        while run:
            ...
            if match == None:
                ...
                if spaces:
                    flat_ann[i] = (flat_ann[i][:match.end() - 1] + "S" +
                                   flat_ann[i][match.end() - 1:])
                else:
...
def tokenize(flat_text, flat_ann, Tokenizer):
    ...
    for i in range(len(flat_text)):
        split_text = [
            Tokenizer.tokenize(preprocess(el, lower=True))
    ...
    for segment in split_combined:
        ...
        for token_set in segment:
            if len(token_set[0]) > 0:
                ...
                if (sum([len(token)
                         for token in token_set[0]]) == len(token_set[1][0])):
                    if len(token_set[1][1]) == 1:
                        ...
                        count = 0
                        for token in token_set[0]:
                            tag = Counter(token_set[1][0][count:(count +
                                                                 len(token))])
